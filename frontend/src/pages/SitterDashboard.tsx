...
const SitterDashboard: React.FC = () => {
    ...
    const filteredBookings = displayedBookings.filter((booking: Booking) => {
        const query = bookingSearch.trim().toLowerCase();
        const ownerName = `${booking.owner?.user?.firstName || ''} ${booking.owner?.user?.lastName || ''}`.toLowerCase();
        const serviceName = booking.serviceType.replace(/([A-Z])/g, ' $1').trim().toLowerCase();
        const dateText = `${booking.startDate} ${booking.endDate}`.toLowerCase();
        const matchesSearch = !query || ownerName.includes(query) || serviceName.includes(query) || dateText.includes(query);
                            ...
                            <CardHeader className="border-b border-slate-100 px-5 py-5 dark:border-slate-800 sm:px-6">
                                <div className="flex flex-col gap-4">
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <div className="flex rounded-xl bg-slate-100 p-1 dark:bg-slate-800">{(['upcoming', 'history'] as const).map((tab) => <button key={tab} onClick={() => setActiveTab(tab)} className={cn('rounded-lg px-3 py-2 text-xs font-semibold transition sm:px-4', activeTab === tab ? 'bg-white text-slate-950 shadow-sm dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-900 dark:hover:text-white')}>{tab === 'upcoming' ? 'Upcoming' : 'History'}</button>)}</div>
                                    </div>
                                    <div className="flex flex-col gap-2 sm:flex-row">
                                        <label className="relative min-w-0 flex-1">
                                            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                            <input value={bookingSearch} onChange={(event) => setBookingSearch(event.target.value)} placeholder="Search by owner, service, or date" className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 text-sm outline-none transition placeholder:text-slate-400 focus:border-primary focus:ring-2 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-800" />
                                        </label>
                                        <select value={bookingStatusFilter} onChange={(event) => setBookingStatusFilter(event.target.value as 'ALL' | BookingStatus)} className="h-10 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-600 outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
                                    ...
                                    {(bookingSearch || bookingStatusFilter !== 'ALL') && <p className="text-xs text-slate-500">Showing {filteredBookings.length} of {displayedBookings.length} bookings</p>}
                                </div>
                            </CardHeader>
